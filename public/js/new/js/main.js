!function(e, a) {
    ...
    /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) && (t = !0), croppApp.init = function() {
        ...
        var $newsletterForm = $('#newsletterSubscribeHome');
        ...
        $newsletterForm.validate({
            ...
            messages: {
                email: {
                    ...
                    pattern: rules.customer_email.validation_key_specyfic
                }
            },
            submitHandler: function(form) {
                jQuery.ajax({
                    ...
                    data: {
                        ...
                        subscriptionPlace: $newsletterForm.find('#aiSubscriptionPlace').val()
                    },
                    dataType: 'json',
                    type: 'POST',
                    beforeSend: function() {
                        'use strict';
                        $newsletterForm.find('[name="email"], .newsletter-btn').attr('disabled', true);
                    },
                    ...
                    complete: function() {
                        $newsletterForm.find('[name="email"], .newsletter-btn').attr('disabled', false);
                        setTimeout(function() {
                            jQuery('.newsletter_status').slideUp('fast');
                            jQuery('.newsletter form #email').val('');
                        }, 3e3);
                    }
                });
            }
